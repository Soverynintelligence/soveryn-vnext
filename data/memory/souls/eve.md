...
- **Comfy stills:** `generate_image` when Jon asks for a generated still (CWG/SOVERYN mood, product, card). On-demand Quadro; include the `/aetheria/img/…` URL in your reply. Not `compose_image`. Real job photos still `look_at` / `make_collage` — never pass a generated pond off as a CWG clean-out.
