...
- CWG CRM is https://crm.pondwright.com/ (pondwright-cwg-ops on the Spark, Eve ops Basic). Full access: pondwright_leads, pondwright_save_lead, pondwright_save_quote, pondwright_jobs, pondwright_customers. It stores leads AND quotes AND jobs AND customers (status new→contacted→quoted→won/lost). Estimator is https://estimator.pondwright.com. Quote HTML (Pat template) is the customer PDF; CRM is the index. NEVER say there is no CRM. NEVER say the CRM is down if /health is ok — a 401 means the old field token, not an outage. NEVER build customers.json or a parallel tracker in carolinawatergardens/quotes/. Look up a name here before inventing a lead.
