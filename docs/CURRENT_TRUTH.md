...
| Night fixer (`soveryn-night-fixer.timer`, 03:30) | **Live 2026-09-24** — first automated dispatch source: nightly suite run, failing test files become templated dispatches (denylist + open-task dedupe, cap 2/night), wake-up note to the CC inbox. Nothing merges without Jon's approve. Critic prose is deliberately NOT parsed (stale findings); this source is deterministic. Live smoke: green suite → quiet note, 0 dispatches |
